module.exports = function (app) {
  ...
  settings.enter((ctx) =>
    ctx.replyWithMarkdown(
      ...
      Markup.keyboard([s_add_canteen, s_del_canteen, "Abbrechen"]) //«
        .oneTime()
        .resize()
        .extra()
    )
  ...
  settings.hears(s_add_canteen, (ctx) =>
    ctx.reply(
      "Wie möchtest du deine Mensa wählen?",
      Markup.keyboard([
        Markup.locationRequestButton("📍 Standort senden"),
        "🔎 Per Name suchen",
      ]) //«
        .oneTime()
        .resize()
  ...
  settings.on("location", (ctx) => {
    fetch(
      "http://openmensa.org/api/v2/canteens?near[lat]=" +
      ...
      .then((json) => {
        if (!error && response.statusCode == 200) {
          ...
          ctx.reply(
            "Ich habe diese Mensen in der Nähe (10km) gefunden. Welche möchtest du hinzufügen?",
  ...
  settings.hears("🥕", (ctx) => {
    ...
    ctx.replyWithMarkdown(
      "Du hast den ultrageheimen Veggie-Filter aktiviert.",
  ...
  settings.hears("🌴", (ctx) => {
    ...
    ctx.replyWithMarkdown(
      "Die Veganer-Polizei überwacht jetzt diesen Speiseplan..",
      Extra.markup(Markup.removeKeyboard())
    );
  ...
  settings.hears("🐖", (ctx) => {
    ...
    ctx.replyWithMarkdown(
      "Du hast den ultrageheimen Veggie-Filter deaktiviert.",
      Extra.markup(Markup.removeKeyboard())
    );
  ...
  settings.hears("🔄🚫", (ctx) => {
    ...
    ctx.replyWithMarkdown(
      "Du hast den ultrageheimen tägliche-Gerichte-Filter aktiviert.",
      Extra.markup(Markup.removeKeyboard())
    );
  ...
  settings.hears("🔄", (ctx) => {
    ...
    ctx.replyWithMarkdown(
      "Du hast den ultrageheimen tägliche-Gerichte-Filter deaktiviert.",
      Extra.markup(Markup.removeKeyboard())
    );
