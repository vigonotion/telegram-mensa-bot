module.exports = function (app) {
  ...
  plan.enter((ctx) => {
    ...
    if (obj !== undefined && obj.canteens.length > 0) {
      sendPlan(
        obj,
        ctx,
        ctx.session.showNotes,
        !ctx.session.showAll,
        ctx.session.editLast
      );
      ...
      ctx.reply(
        "Du hast noch keine Mensa eingestellt. Benutze /settings zum einstellen der Mensa."
      );
    }
  ...
  function sendPlan(
    ...
    if (ctx.session.currentCanteen === undefined)
      ctx.session.currentCanteen = 0;
    ctx.session.currentCanteen = modulo(
    ...
    fetch(
      ...
      .then((json) => {
        ...
        function keyboard(
          ...
          keyboards = [];
          canteen_nav = [
            m.callbackButton("« Vorherige Mensa", "last_canteen"),
            m.callbackButton("Nächste Mensa »", "next_canteen"),
          ];
          if (notes)
            keyboards.push(m.callbackButton("Inhaltsstoffe anzeigen", "notes"));
          if (more)
            keyboards.push(
              m.callbackButton(morecount + " weitere anzeigen", "unfiltered")
            );
        ...
        let md = Extra.markdown().markup((m) =>
          m.inlineKeyboard(
            keyboard(
              m,
              obj.canteens.length > 1,
              !notes,
              morecount > 0,
              morecount
            )
        ...
        if (editLast) ctx.editMessageText(response, md);
        else ctx.reply(response, md);
      });
  }
