module.exports = function(app) {
  ...
  canteen_delete.hears(/(.+)#(\d+)/gm, (ctx) => {
    ...
    ctx.replyWithMarkdown("_" + ctx.match[1] + "_ wurde von deinen aktiven Mensen entfernt.", Extra.markup(Markup.removeKeyboard()))
  ...
  function fn_delete_canteen(ctx) {
    ...
    let user_canteens = app.db.get('users')
      .find({ user_id: ctx.from.id })
      .get('canteens')
      .value()
    ...
    let results = [];
      ...
      ctx.reply('Welche Mensa möchtest du entfernen?', Markup
          .keyboard(results) //«
          .oneTime()
          .resize()
          .extra()
